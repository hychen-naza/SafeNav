...
class LaserData:
    def __init__(self, depth: float, theta: float, rel_x: float, rel_y: float, v_x: float, v_y: float):
        self.x = rel_x
        self.y = rel_y
        self.vx = v_x
        self.vy = v_y
...
class EgoCircle:
    ...
    def parse_sensor_data(self, sensor_data: dict):
        ...
        for _, obs_state in obstacle_states.items():
            ...
            if (angle not in self.depths):
                self.depths[angle] = []
            self.depths[angle].append(LaserData(depth, angle, rel_x, rel_y, robot_vel_state[0], robot_vel_state[1]))
    ...
    def build_gaps(self, sensor_data: dict):
        ...
        if (len(angles) == 0):
            ...
            gap = Gap(goal_rel_pos[0], goal_rel_pos[1])
            gap.set_score(0)
            gaps.append(gap)
        # one obstacle
        elif (len(angles) == 1):
            ...
            for i in range(1, num_gap+1):
                ...
                rel_x = laser_data.depth * math.sin(fake_theta * 2 * math.pi / 360)
                rel_y = laser_data.depth * math.cos(fake_theta * 2 * math.pi / 360)
                self.inflated_depths[fake_theta] = LaserData(laser_data.depth, fake_theta, rel_x, rel_y, 0, 0)
            ...
            for angle1, angle2 in zip(angle_start, angle_end):  
                ...
                if (num_gap >= 2):
                    for i in range(1, num_gap):
                        ...
                        angles.append(fake_theta)
                        depth = laser_data1.depth + ((i * angle) / (diff_angle) * (laser_data2.depth - laser_data1.depth))
                        rel_x = depth * math.sin(fake_theta * 2 * math.pi / 360)
                        rel_y = depth * math.cos(fake_theta * 2 * math.pi / 360)
                        self.inflated_depths[fake_theta] = LaserData(depth, fake_theta, rel_x, rel_y, 0, 0)
            angles.sort()  
            gaps = self.build_gap_from_angles(angles, robot_state)
        self.inflated_depths = {}
        self.depths = {}
    ...
    def build_gap_from_angles(self, angles, robot_state):
        ...
        for angle1, angle2 in zip(angle_start, angle_end):                
            ...
            # Large interval
            if (arc_dist > 2*self.safety_dist):                                
